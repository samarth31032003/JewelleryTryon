...
# --- SHADERS (Taken from your shaders.py) ---
VERTEX_SHADER = """
#version 330 core
...
void main() {
    if (u_has_tex) {
        ...
        // Fallback color if no texture (Yellow-ish)
...
class ModernMeshWidget(QOpenGLWidget):
    ...
    def initializeGL(self):
        """Compiles shaders and sets up global GL state."""
    ...
    def _init_uniforms(self):
        """Finds the memory addresses of our shader variables."""
    ...
    def load_mesh(self, file_path):
        """Loads mesh using the robust logic from your utils.py"""
        try:
            # FORCE='mesh' handles GLB scenes automatically
            mesh = trimesh.load(file_path, force='mesh')
            
            # --- Auto-Scaling (From utils.py) ---
            mesh.vertices -= mesh.centroid # Center it
            max_dim = np.max(mesh.bounding_box.extents)
            if max_dim > 0:
                mesh.apply_scale(1.5 / max_dim) # Scale to fit view
            
            # --- Texture Loading (From utils.py) ---
            self.makeCurrent() # GL context must be active
    ...
    def _load_texture_data(self, mesh):
        """Extracts texture, flips it (CRITICAL), and uploads to GPU."""
        tex_img = None
        # 1. Try standard material image
        if hasattr(mesh.visual, 'material') and hasattr(mesh.visual.material, 'image'):
            tex_img = mesh.visual.material.image
        # 2. Try direct visual image (sometimes happens in simple OBJs)
        elif hasattr(mesh.visual, 'image'):
            tex_img = mesh.visual.image
            
        if tex_img:
            # Convert to PIL if it isn't already
            ...
            # CRITICAL FIX: Flip image upside down for OpenGL
    ...
    def _upload_geometry(self, mesh):
        """Uploads Vertices, Normals, and UVs to VBOs."""
        # Prepare Data
        ...
        # Attribute Pointers
        stride = data.strides[0] # Bytes per vertex row
        # Position (0)
        glEnableVertexAttribArray(0)
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, stride, ctypes.c_void_p(0))
        # Normal (1) - offset 12 bytes (3 floats)
        glEnableVertexAttribArray(1)
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, stride, ctypes.c_void_p(12))
        # UV (2) - offset 24 bytes (6 floats)
    ...
    def resizeGL(self, w, h):
        ...
        # Create Perspective Matrix manually since we are using shaders
    ...
    def paintGL(self):
        ...
        # Move camera back a bit (View Matrix)
        view = np.eye(4, dtype=np.float32)
        view[3, 2] = -3.0 # Translate Z -3
    ...
    # --- Math Helpers (Since we aren't using legacy glu functions) ---
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Modern OpenGL Viewer (OBJ + GLB)")
        self.resize(800, 600)
    ...
    def load_model(self):
        fname, _ = QFileDialog.getOpenFileName(self, "Open", "", "3D Files (*.glb *.obj)")
        if fname:
            self.gl_widget.load_mesh(fname)
